...
# VERSİYON
# =========================================================
VERSION_NAME = "Balina Avcısı V6.1 MEXC ANA VERİ + MEXC OI/FUNDING + WS/CVD/SPOOF + 15M MA GUARD V5 - DEEPSEEK AÇIK"
...
def _mexc_kline_to_rows(data: Dict[str, Any], limit: int) -> List[List[Any]]:
    ...
    for i in range(n):
        ts_ms = int(safe_float(times[i], 0) * 1000)
        rows.append([
            str(ts_ms),
            str(opens[i]),
            str(highs_[i]),
...
async def mexc_public_ws_loop() -> None:
    ...
    while True:
        try:
            ...
            async with websockets.connect(MEXC_WS_URL, ping_interval=None, ping_timeout=None, close_timeout=10, max_size=4_000_000) as ws:
                ws_runtime_state.update({"connected": True, "last_error": "-", "subscriptions": len(symbols) * 2})
                # MEXC her sembol için depth + deal aboneliği
                for sym in symbols:
                    msym = normalize_mexc_symbol(sym)
                    await ws.send(json.dumps({"method": "sub.depth", "param": {"symbol": msym}, "gzip": False}))
                    await ws.send(json.dumps({"method": "sub.deal", "param": {"symbol": msym}, "gzip": False}))
                logger.info("MEXC WS bağlandı | sembol=%s | kanal=%s", len(symbols), len(symbols) * 2)
